    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('month') ? 'is-invalid' : '' }}" name="month" id="month">
                    ...
                    @foreach(App\Models\Payment::MONTH_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('month', $payment->month) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('month'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" name="year" id="year">
                    ...
                    @foreach(App\Models\Payment::YEAR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('year', $payment->year) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('year'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('received_by') ? 'is-invalid' : '' }}" type="text" name="received_by" id="received_by" value="{{ old('received_by', $payment->received_by) }}">
